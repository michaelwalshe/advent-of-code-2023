...
def _get_cookie_headers(root: Path) -> dict[str, str]:
    with (root / ".env").open() as f:
        contents = f.read().strip()
    return {"Cookie": contents}


def get_input(year: int, day: int, root: Path) -> str:
    url = f"https://adventofcode.com/{year}/day/{day}/input"
    req = urllib.request.Request(url, headers=_get_cookie_headers(root))
    return urllib.request.urlopen(req).read().decode()


def download_input(day: int, year: int, daypath: Path) -> int:
    parser = argparse.ArgumentParser()
    parser.parse_args()

    for i in range(5):
        try:
            s = get_input(year, day, daypath.parent)
